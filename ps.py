    ...
    for rb in range(0, 256):
        for rc in range(0, 256):
            for rd in range(0, 256):
                ...
                # Command to assing a load database into memory, faster
                # performance but uses more memory
                gi = GeoIP.new(GeoIP.GEOIP_MEMORY_CACHE)

                # México code (MX) comparision to extract just "in México"
                # geolocalizated IPs.
                if gi.country_code_by_addr(a + '.' + b + '.' + c + '.' + d) == 'MX':

                    # Command to writte the IP on the manipulated file
                    # (data_mx.txt).
                    file.write(a + '.' + b + '.' + c + '.' + d + '\r\n')
...
# Command to print calculated execution time in minutes (et/60) and hours
# (et/60/60).
